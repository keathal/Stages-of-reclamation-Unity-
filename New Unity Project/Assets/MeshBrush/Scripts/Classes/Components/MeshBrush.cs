...
namespace MeshBrush
{
    ...
    public class MeshBrush : MonoBehaviour
    {
        ...
        /// </summary>
        public Vector2 randomRotationRange = new Vector2(0.0f, 5.0f);
        ...
        public void OnValidate()
        {
            ...
            VectorClampingUtility.ClampVector(ref randomRotationRange, 0.0f, 100.0f, 0.0f, 100.0f);
            ...
            for (int i = numberOfMeshesToPaint; i > 0; i--)
            {
                ...
                if (globalPaintingMode ? Physics.Raycast(new Ray(brushTransform.position, -brushLocation.normal), out tempHit, 2.5f) : CachedCollider.Raycast(new Ray(brushTransform.position, -brushLocation.normal), out tempHit, 2.5f))
                {
                    ...
                    if (inverseSlopeFilter ? slopeAngle > Random.Range(angleThresholdRange.x, angleThresholdRange.y) : slopeAngle < Random.Range(angleThresholdRange.x, angleThresholdRange.y))
                    {
                        ...
                        paintedMesh = (GameObject)PrefabUtility.InstantiatePrefab(meshes[Random.Range(0, meshes.Count)]);
#else
                        ...
                        if (randomRotationRange.x > 0.0f || randomRotationRange.y > 0.0f)
                        {
                            MeshTransformationUtility.ApplyRandomRotation(paintedMeshTransform, Random.Range(randomRotationRange.x, randomRotationRange.y));
                        }
            ...
            for (int i = paintedMeshesInsideBrushArea.Count - 1; i >= 0; i--)
            {
                ...
                if (transformToRandomize != null)
                {
                    ...
                    if (rotationBrushRandomizer)
                    {
                        ...
                        if (randomRotationRange.x > 0.0f || randomRotationRange.y > 0.0f)
                        {
                            MeshTransformationUtility.ApplyRandomRotation(transformToRandomize, Random.Range(randomRotationRange.x, randomRotationRange.y));
                        }
                    }
...
#if UNITY_EDITOR
                Undo.DestroyObjectImmediate(paintedMeshesInsideBrushArea[i].gameObject);
#else
                Destroy(paintedMeshesInsideBrushArea[i].gameObject);
#endif
            }
        }
        ...
        public void ResetRandomizers()
        {
            ...
            randomRotationRange = new Vector2(0.0f, 5.0f);
                ...
                new XElement("meshBrushTemplate", new XAttribute("version", version),
                    ...
                    new XElement("randomizers",
                        ...
                        new XElement("rotation",
                            new XElement(minString, randomRotationRange.x),
                            new XElement(maxString, randomRotationRange.y)),
                        ...
                        foreach (var subElement in element.Elements())
                        {
                            switch (subElement.Name.LocalName)
                            {
                                ...
                                case "rotation":
                                    ...
                                    {
                                        randomRotationRange = new Vector2(float.Parse(subElement.Element(minString).Value), float.Parse(subElement.Element(maxString).Value));
                                    }
                                    break;
